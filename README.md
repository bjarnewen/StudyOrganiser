...
- **Widgets** — today's classes on the iPad Home Screen and the Mac desktop (see [Widgets](#widgets))
...
cd web && npm test              # 87 logic tests, Node's built-in runner
cd web && npm run build:widgets # regenerate the widgets after editing their source
...
web/
  ...
  test/            parser, importer, store, merge and widget tests
widgets/
  src/             widget sources; agenda-core.js is shared by both
  *.js *.jsx       built widgets, regenerated by scripts/build-widgets.mjs
```
...
not a gap in this code. So the widgets are small scripts for two free widget hosts. Both read the same
private gist the app syncs to, so they always agree with the app and need no server of their own.

Both are generated from one source (`widgets/src/agenda-core.js`) so they can't drift apart. After
changing that file, run `node scripts/build-widgets.mjs`; CI fails if the committed copies are stale.

### iPad / iPhone — Scriptable

1. Install **[Scriptable](https://apps.apple.com/app/scriptable/id1405459188)** from the App Store (free).
2. On the device, open
   `https://bjarnewen.github.io/StudyOrganiser/widgets/StudyOrganiser.scriptable.js`, select all, copy.
3. In Scriptable, tap **+**, paste, and name it **Study Organiser**.
4. Run it once (▶). It asks for your GitHub token — the same `gist`-scoped one the app uses. It finds
   your gist by itself and stores the token in the iOS keychain, not in the script.
